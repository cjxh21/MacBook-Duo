...
唤醒使用独立的透视开盖渲染：画面从黑场绕屏幕下沿展开，翻转幅度按当前开屏角度计算（120° 对应 60°，150° 对应 30°），由模糊逐渐变清晰，前段展开、后段缓慢收尾，轮廓按玻璃软边渐隐，结束帧与正常画面一致。真实铰链运动继续使用透视与磨砂效果。桌面会在息屏时准备黑色过渡层，并提前编译渲染器；新画面完成 GPU 绘制后再交给动画。唤醒动画无需等待铰链采样恢复，超过 2 秒仍未取得首帧则放弃这次动画，避免迟到后突然接管。
