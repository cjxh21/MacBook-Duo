...
角度桥使用扩展 Documents 下 `duo-state.bin`，72 字节 mmap + flock；主 App 只打开扩展已创建的文件，不主动伪造沙箱容器。墙纸读取正式版同一份校准、预测、磨砂和边缘柔和度设置。睡眠时不提交绘制，但保留轻量计时器等待唤醒。
...
检测到屏幕休眠后再次唤醒，且锁屏时新鲜角度数据表明盖子已到校准的清晰区，会播放一次 0.9 秒的自动翻开动画（沿用现有透视翻页渲染），随后恢复清晰壁纸。重复唤醒通知不会重播；实际掀盖继续跟随传感器；解锁、再次休眠或角度离开清晰区会取消动画。等待锁屏和新鲜数据最多 2 秒，超时放弃。黑屏期间暂停提交帧，唤醒时清空旧队列。
