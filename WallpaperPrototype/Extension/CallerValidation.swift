...
enum CallerValidation {
    static func isAcceptable(_ connection: NSXPCConnection) -> Bool {
        guard connection.responds(to: NSSelectorFromString("auditToken")) else {
            extensionLog("  [Caller] auditToken SPI unavailable — rejecting")
            return false
        }
        ...
        guard status == errSecSuccess, let code else {
            extensionLog("  [Caller] Could not derive peer SecCode (status \(status)) — rejecting")
            return false
        }

        guard let appleReq = requirement("anchor apple") else {
            extensionLog("  [Caller] Could not build anchor-apple requirement — rejecting")
            return false
        }
