...
struct SettingsItem: Codable {
    var id: ChoiceID
    // WallpaperSettingsItem carries the owning group on every selectable
    // item.  System-provided wallpaper extensions include this even when the
    // group already has the same identifier; omitting it leaves a tile
    // visible in Settings but gives WallpaperAgent no selection target.
    var groupID: GroupID?
    var subgroupID: GroupID?
    var localizedName: String
